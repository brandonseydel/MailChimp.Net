...
internal class CampaignLogic : BaseLogic, ICampaignLogic
    ...
    public async Task<Campaign> AddAsync(Campaign campaign) => await CreateAsync(campaign).ConfigureAwait(false);
    ...
    public async Task<IEnumerable<Campaign>> GetAll(CampaignRequest request = null) => (await GetResponseAsync(request).ConfigureAwait(false))?.Campaigns;
    ...
    /// <exception cref="NotSupportedException"><paramref name="element" /> is not a constructor, method, property, event, type, or field. </exception>
    /// <exception cref="TypeLoadException">A custom attribute type cannot be loaded. </exception>
    public async Task<IEnumerable<Campaign>> GetAllAsync(CampaignRequest request = null) => (await GetResponseAsync(request).ConfigureAwait(false))?.Campaigns;
